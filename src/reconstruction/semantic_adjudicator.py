#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""LLM semantic adjudication over traceable drawing evidence."""
...
class SemanticAdjudicationValidator:
    """Validate the semantic adjudication handoff before later stages consume it."""
...
class LLMSemanticAdjudicator:
    """Ask the model to adjudicate view, dimension, and feature semantics."""
...
输出必须是合法 JSON 对象，格式示例：
{
  ...
  "warnings": []
    ...
    def adjudicate(
        ...
        if self.config.get("semantic_adjudication_thinking", False):
            request_payload["extra_body"] = {
                "thinking": {
                    "type": "enabled",
                    "reasoning_effort": self.config.get(
                        "semantic_adjudication_reasoning_effort",
                        "medium",
                    ),
                }
            }
